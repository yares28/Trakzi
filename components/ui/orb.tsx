...
function Scene({
  ...
  useFrame((_, delta: number) => {
    ...
    // Start with full opacity immediately for better visibility
    if (u.uOpacity.value < 1) {
      u.uOpacity.value = Math.min(1, u.uOpacity.value + delta * 5)
    }
  ...
  const uniforms = useMemo(() => {
    ...
    return {
      uColor1: { value: new THREE.Color(initialColorsRef.current[0]) },
      uColor2: { value: new THREE.Color(initialColorsRef.current[1]) },
  ...
  // Remove error checking that's causing issues - Three.js will handle shader errors
