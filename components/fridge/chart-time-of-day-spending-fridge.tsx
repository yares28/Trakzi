...
type TimeOfDaySpendDatum = {
  timeOfDay: string
  spend: number
}

interface ChartTimeOfDaySpendingFridgeProps {
  data?: TimeOfDaySpendDatum[]
  isLoading?: boolean
}
...
export function ChartTimeOfDaySpendingFridge({ data = [], isLoading = false }: ChartTimeOfDaySpendingFridgeProps) {
  ...
  const renderInfoTrigger = () => (
    <div className="flex flex-col items-center gap-2">
      <ChartInfoPopover
        title="Time of Day Spending"
        description="Compare total spending across different times of day."
        details={[
          "Each bar represents total spending for that time period.",
      ...
      <ChartAiInsightButton
        ...
        chartTitle="Time of Day Spending"
        chartDescription="Compare total spending across different times of day."
        chartData={{
          totals: data,
          topTime: data.reduce(
  ...
  const option = React.useMemo(() => {
    ...
    // Sort data by time period order
    const sortedData = [...data].sort((a, b) => {
      const aIndex = TIME_PERIOD_ORDER.indexOf(a.timeOfDay)
      const bIndex = TIME_PERIOD_ORDER.indexOf(b.timeOfDay)
    ...
    sortedData.forEach((item) => {
      datasetSource.push([item.timeOfDay, item.spend])
    })
