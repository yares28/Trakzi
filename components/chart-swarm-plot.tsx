...
const SwarmInfoTrigger = memo(function SwarmInfoTrigger({
  swarmControls,
  forFullscreen = false,
}: SwarmInfoTriggerProps) {
  return (
    <div
  ...
  return (
    <ResponsiveSwarmPlot
      ...
      axisBottom={
        isMobileSize
          ? { tickRotation: -45 }
          : { legend: "category vs. amount", legendOffset: 40 }
      }
...
export const ChartSwarmPlot = memo(function ChartSwarmPlot({
  data,
  emptyTitle,
  emptyDescription,
}: ChartSwarmPlotProps) {
  ...
  const {
    ...
    error: swrError,
  } = useSWR<ChartSwarmPlotDatum[]>(
    data === undefined ? "/api/charts/transaction-history" : null,
    swarmFetcher,
  );
  const remoteData: ChartSwarmPlotDatum[] =
    data === undefined && Array.isArray(swrData) ? swrData : [];
  const isLoading = data === undefined ? swrLoading : false;
  const error: string | null = swrError
    ? swrError?.message || "Failed to load transactions"
    : null;

  // Fetch categories for filter options
  const { data: categoriesRaw } = useSWR<
    { name?: string; totalSpend?: number | string }[]
  >("/api/categories", categoriesFetcher);
  ...
  const maxCategories = useMemo(
    () => getMaxCategoriesForWidth(containerWidth),
    [containerWidth],
  );
    ...
    return (
      sourceData
        .map((item) => {
          ...
          const categoryLabel = normalizeCategoryValue(
            item.category || item.group || "Other",
          );
  ...
  const chartGroups = useMemo(() => {
    ...
    const filteredCombined = combinedCategoryOptions.filter((cat) =>
      categoriesWithData.has(cat),
    );
    ...
    const filteredFallback = fallbackCategoryOptions.filter((cat) =>
      categoriesWithData.has(cat),
    );
  ...
  const filteredData = useMemo(() => {
    ...
    return sanitizedData.filter((item) =>
      visibleGroups.includes(item.categoryLabel),
    );
  ...
  const dynamicValueScale = useMemo(() => {
    ...
    const sortedPrices = filteredData
      .map((item) =>
        Number.isFinite(item.price) ? Math.max(0, item.price) : 0,
      )
  ...
  return (
    <>
      <ChartFullscreenModal
        ...
        headerActions={
          <SwarmInfoTrigger swarmControls={swarmControls} forFullscreen />
        }
      >
        <div className="h-full w-full min-h-[400px]">
          <SwarmChartContent
            ...
            dynamicValueScale={dynamicValueScale}
            swarmTheme={swarmTheme}
