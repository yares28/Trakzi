...
import type { LinkTransactionsRequest, UnlinkTransactionsRequest } from '@/lib/types/pockets'

/**
 * POST /api/pockets/links
...
export async function POST(request: Request) {
    try {
        ...
        if (updatedCount < transaction_ids.length) {
            console.warn(`[Pockets Links] Only ${updatedCount} of ${transaction_ids.length} transactions updated for user ${userId}`)
        }
