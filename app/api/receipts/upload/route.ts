...
async function repairReceiptJsonWithAi(params: {
  ...
  const prompt = [
    ...
    "- Energy drinks/soda/sugary drinks → proper beverage category (NOT Other)",
    "- Plain water → \"Water\"",
    "- Coffee/tea (unsweetened) → \"Coffee & Tea\"",
    "- Milk/yogurt (not cheese) → \"Dairy (Milk/Yogurt)\"",
    "- Try to assign a specific category. Use \"Other\" ONLY when truly uncertain or item doesn't fit any category.",
    "",
    schemaPrompt,
    "",
