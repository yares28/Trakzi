    ...
    try {
        ...
        if (plan === 'free') {
            return NextResponse.json(
                { error: 'Upgrade to Pro or Max to access savings analytics', upgradeRequired: true },
                { status: 403 }
            )
        }
        ...
        console.error('[Savings Bundle API] Error:', error)
        return NextResponse.json(
            { error: error.message || 'Failed to fetch savings bundle' },
            { status: 500 }
        )
    }
}
