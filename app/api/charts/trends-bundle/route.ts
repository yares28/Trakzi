...
import { getTrendsBundle, type TrendsSummary } from '@/lib/charts/home-trends-savings-aggregations'
    ...
    try {
        ...
        if (plan === 'free') {
            return NextResponse.json(
                { error: 'Upgrade to Pro or Max to access trends analytics', upgradeRequired: true },
                { status: 403 }
            )
        }
        ...
        // Build cache key
        const cacheKey = buildCacheKey('trends', userId, filter, 'bundle')

        // Try cache first, otherwise compute
        const data = await getCachedOrCompute<TrendsSummary>(
            cacheKey,
            () => getTrendsBundle(userId!, filter),
            CACHE_TTL.analytics
        )
        ...
        console.error('[Trends Bundle API] Error:', error)
        return NextResponse.json(
            { error: error.message || 'Failed to fetch trends bundle' },
            { status: 500 }
        )
    }
}
