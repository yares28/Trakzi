...
export async function GET(request: Request) {
  try {
    ...
    const cacheKey = buildCacheKey('financial-health', userId, filter ?? undefined, 'radar')
    const result = await getCachedOrCompute<FinancialHealthResponse>(
      cacheKey,
      () => computeFinancialHealth(userId, filter),
      CACHE_TTL['financial-health']
    )

    return NextResponse.json<FinancialHealthResponse>(result)
  } catch (error) {
