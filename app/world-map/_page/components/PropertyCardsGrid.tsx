...
import { Home, Plus, Eye, X, Loader2, Pencil, Check, X as XIcon } from "lucide-react"
...
const MOCK_PROPERTIES: PropertyCardData[] = [
  { id: "1", label: "Main Home", value: 420000 },
  { id: "2", label: "Rental A", value: 285000 },
  { id: "3", label: "Cabin", value: 125000 },
]
...
function PropertyCard({ id, label, value, onView, onRemove, onLabelUpdated }: PropertyCardProps) {
...
export const PropertyCardsGrid = memo(function PropertyCardsGrid() {
  const [properties, setProperties] = useState<PropertyCardData[]>(MOCK_PROPERTIES)
    ...
    <div className="grid grid-cols-1 gap-4 px-4 lg:px-6 @md/main:grid-cols-2 @3xl/main:grid-cols-3">
      {properties.map((p) => (
        <PropertyCard
          ...
          onLabelUpdated={(newLabel) => handleLabelUpdated(p.id, newLabel)}
        />
      ))}
    </div>
  )
