    ...
    <div className="relative z-10">
      ...
      <section className="py-24 px-6">
        <m.div initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }} transition={{ duration: 0.5 }} className="max-w-3xl mx-auto">
          <div className="inline-flex items-center gap-2 px-4 py-2 rounded-full bg-foreground/5 border border-foreground/10 mb-6">
            <span className="text-primary font-medium text-sm">Trakzi Docs</span>
          </div>
        ...
        <div className="max-w-3xl mx-auto space-y-6">
          {posts.map((post, i) => (
            <m.div key={post.slug} initial={{ opacity: 0, y: 15 }} animate={{ opacity: 1, y: 0 }} transition={{ duration: 0.4, delay: i * 0.08 }}>
              <Link
                href={`/docs/${post.slug}`}
                className="group block p-6 rounded-2xl border border-foreground/10 bg-foreground/5 hover:border-primary/60 transition-all"
              >
        ...
        <div className="max-w-3xl mx-auto">
          ...
          <div className="grid sm:grid-cols-3 gap-4">
            {[
              ...
              <Link key={c.href} href={c.href} className="group block p-5 rounded-2xl border border-foreground/10 bg-foreground/5 hover:border-primary/60 transition-all">
      ...
      <section className="pb-24 px-6">
        <div className="max-w-3xl mx-auto text-center">
          <h2 className={cn("text-2xl font-semibold text-foreground mb-3", geist.className)}>Ready to take action?</h2>
