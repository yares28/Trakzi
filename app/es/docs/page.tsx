...
import { ArrowRight, Clock, Calendar, Sparkles, Globe } from "lucide-react"
...
export default function EsDocsIndexPage() {
  return (
    <div className="relative z-10">
      ...
      <section className="relative min-h-screen flex flex-col">
        ...
        <div className="container mx-auto px-4 py-24 sm:py-32 relative z-10 flex-1 flex flex-col">
          <div className="mx-auto max-w-4xl text-center flex-1 flex flex-col justify-center relative">
            ...
            >
              Guías y <strong>Documentación.</strong>
            </m.h1>
            ...
            <m.div initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }} transition={{ duration: 0.5, delay: 0.3 }} className="flex flex-col items-center gap-6">
              <svg width="100" height="50" viewBox="0 0 100 50" fill="currentColor" xmlns="http://www.w3.org/2000/svg" className="text-foreground">
          ...
          <div className="flex-1 min-w-0">
            ...
            <section className="pb-16">
              <div className="space-y-4">
                <div className="border-primary/40 text-primary inline-flex items-center gap-2 rounded-full border px-3 py-1 uppercase mb-8">
                  <span>✶</span>
                ...
                {esPosts.map((post, i) => (
                  <m.div key={post.slug} initial={{ opacity: 0, y: 15 }} animate={{ opacity: 1, y: 0 }} transition={{ duration: 0.4, delay: i * 0.08 }}>
                    <Link
                      href={`/es/docs/${post.slug}`}
                      className="group block p-6 rounded-2xl border border-white/10 bg-card shadow-[0px_2px_0px_0px_rgba(255,255,255,0.1)_inset] hover:border-white/20 transition-all"
                    >
                      <div className="flex flex-wrap gap-2 mb-3">
                        {post.tags.map((tag) => (
                          <span key={tag} className="text-[10px] font-medium text-primary border border-primary/40 px-2 py-0.5 rounded-full">
                            {tag}
                          </span>
                        ))}
                      </div>
                      <h2 className="text-2xl leading-none font-semibold tracking-tight text-foreground mb-2 group-hover:text-primary transition-colors">
                        {post.title}
                      </h2>
            ...
            <section className="pb-16">
              ...
              <div className="grid sm:grid-cols-3 gap-4">
                {[
                  ...
                  <Link key={c.href} href={c.href} className="group block p-5 rounded-2xl border border-white/10 bg-card shadow-[0px_2px_0px_0px_rgba(255,255,255,0.1)_inset] hover:border-white/20 transition-all">
                    <h3 className="text-sm font-semibold text-foreground mb-1 group-hover:text-primary transition-colors">{c.name}</h3>
                    ...
                    <div className="mt-2 flex items-center gap-1 text-primary text-[10px] opacity-0 group-hover:opacity-100 transition-opacity">
                      Comparar <ArrowRight className="h-2.5 w-2.5" />
                    </div>
                  </Link>
                ))}
              </div>
      ...
      {/* CTA */}
      <section className="relative overflow-hidden pt-24 pb-96 px-6 z-10">
        ...
        <div className="max-w-3xl mx-auto text-center relative z-10">
          <m.h2
            className={cn("text-5xl md:text-[72px] md:leading-[80px] font-semibold tracking-tighter mb-4", geist.className)}
            initial={{ opacity: 0, y: 20 }}
          ...
          <m.p
            ...
            viewport={{ once: true }}
          >
            Pon estas guías en práctica con Trakzi. Gratis para empezar.
          </m.p>
          <m.div
            initial={{ opacity: 0, y: 20 }}
            whileInView={{ opacity: 1, y: 0 }}
