...
import type { UnlinkedTransactionsResponse, CountryTransaction, CountryInstancesResponse } from "@/lib/types/pockets"
...
export const AddCountryDialog = memo(function AddCountryDialog({
    ...
    const { data, isLoading } = useSWR<UnlinkedTransactionsResponse>(
        open ? `/api/pockets/unlinked-transactions` : null,
        fetcher
    )
    ...
    const { data: existingInstances } = useSWR<CountryInstancesResponse>(
        open && selectedCountry ? `/api/pockets/instances` : null,
        fetcher
    )
    ...
    const handleSubmit = async () => {
        ...
        try {
            ...
            const createResponse = await fetch('/api/pockets/instances', {
                method: 'POST',
